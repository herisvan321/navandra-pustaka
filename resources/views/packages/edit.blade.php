...
<div class="flex items-center justify-between">
    <div>
        ...
        <h2 class="text-3xl font-display font-black text-navy leading-tight">
            Edit <span class="text-gold">Paket</span>
        </h2>
    </div>
</div>
@endsection
...
@section('content')
<div class="max-w-4xl">
    <form action="{{ route('packages.update', $package) }}" method="POST" class="bg-white rounded-2xl-custom border border-off-white shadow-custom p-8">
        @csrf
        @method('PUT')
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div class="space-y-6">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $package->name) }}" class="w-full px-4 py-3 rounded-xl border border-off-white bg-off-white/30 focus:outline-none focus:ring-2 focus:ring-gold/20 focus:border-gold transition-all" placeholder="Contoh: Paket Digital">
                    @error('name') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label class="block text-xs font-black uppercase tracking-widest text-navy mb-2">Tagline</label>
                    <input type="text" name="tagline" value="{{ old('tagline', $package->tagline) }}" class="w-full px-4 py-3 rounded-xl border border-off-white bg-off-white/30 focus:outline-none focus:ring-2 focus:ring-gold/20 focus:border-gold transition-all" placeholder="Contoh: Ideal untuk penulis pemula">
                ...
                <div>
                    ...
                    <input type="number" name="price" value="{{ old('price', $package->price) }}" class="w-full px-4 py-3 rounded-xl border border-off-white bg-off-white/30 focus:outline-none focus:ring-2 focus:ring-gold/20 focus:border-gold transition-all">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" name="order" value="{{ old('order', $package->order) }}" class="w-full px-4 py-3 rounded-xl border border-off-white bg-off-white/30 focus:outline-none focus:ring-2 focus:ring-gold/20 focus:border-gold transition-all">
                    </div>
                    <div>
                        ...
                        <select name="is_featured" class="w-full px-4 py-3 rounded-xl border border-off-white bg-off-white/30 focus:outline-none focus:ring-2 focus:ring-gold/20 focus:border-gold transition-all">
                            <option value="0" {{ old('is_featured', $package->is_featured) == '0' ? 'selected' : '' }}>Biasa</option>
                            <option value="1" {{ old('is_featured', $package->is_featured) == '1' ? 'selected' : '' }}>Terpopuler</option>
                        </select>
                    </div>
                </div>
            </div>
            ...
            <div>
                ...
                <div id="features-container" class="space-y-3">
                    ...
                    <div class="flex gap-2">
                        <input type="text" name="features[]" value="{{ $feature }}" class="flex-1 px-4 py-2 rounded-lg border border-off-white bg-off-white/30 focus:outline-none focus:ring-2 focus:ring-gold/20 focus:border-gold transition-all" placeholder="Fitur baru...">
        ...
        <div class="mt-8 flex justify-end gap-3 border-t border-off-white pt-8">
            ...
            <button type="submit" class="px-8 py-3 bg-gradient-to-r from-navy to-navy-light text-white font-bold text-xs rounded-xl hover:shadow-lg transition-all border border-gold/10 uppercase tracking-widest">Perbarui Paket</button>
        </div>
    </form>
</div>
