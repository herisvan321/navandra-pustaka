...
<div class="flex items-center justify-between">
    <div>
        ...
        <h2 class="text-3xl font-display font-black text-navy leading-tight">
            Tambah <span class="text-gold">Koleksi</span> Baru
        </h2>
    </div>
</div>
@endsection
...
@section('content')
<div class="max-w-3xl">
    <form action="{{ route('books.store') }}" method="POST" class="bg-white rounded-2xl-custom border border-off-white shadow-custom p-8">
        @csrf
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="md:col-span-2">
                <label class="block text-xs font-black uppercase tracking-widest text-navy mb-2">Judul Buku</label>
                <input type="text" name="title" value="{{ old('title') }}" class="w-full px-4 py-3 rounded-xl border border-off-white bg-off-white/30 focus:outline-none focus:ring-2 focus:ring-gold/20 focus:border-gold transition-all" placeholder="Masukkan judul buku...">
                @error('title') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
            </div>

            <div>
                <label class="block text-xs font-black uppercase tracking-widest text-navy mb-2">Penulis</label>
                <input type="text" name="author" value="{{ old('author') }}" class="w-full px-4 py-3 rounded-xl border border-off-white bg-off-white/30 focus:outline-none focus:ring-2 focus:ring-gold/20 focus:border-gold transition-all" placeholder="Nama penulis...">
            ...
            <div>
                ...
                <input type="text" name="isbn" value="{{ old('isbn') }}" class="w-full px-4 py-3 rounded-xl border border-off-white bg-off-white/30 focus:outline-none focus:ring-2 focus:ring-gold/20 focus:border-gold transition-all" placeholder="Contoh: 978-602-...">
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="description" rows="5" class="w-full px-4 py-3 rounded-xl border border-off-white bg-off-white/30 focus:outline-none focus:ring-2 focus:ring-gold/20 focus:border-gold transition-all" placeholder="Tuliskan ringkasan buku...">{{ old('description') }}</textarea>
        ...
        <div class="mt-8 flex justify-end gap-3">
            ...
            <button type="submit" class="px-8 py-3 bg-gradient-to-r from-navy to-navy-light text-white font-bold text-xs rounded-xl hover:shadow-lg transition-all border border-gold/10 uppercase tracking-widest">Simpan Koleksi</button>
        </div>
    </form>
</div>
@endsection
