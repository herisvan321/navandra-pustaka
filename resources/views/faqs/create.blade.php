...
<div class="flex items-center justify-between">
    <div>
        ...
        <h2 class="text-3xl font-display font-black text-navy leading-tight">
            Tambah <span class="text-gold">FAQ</span> Baru
        </h2>
    </div>
</div>
@endsection
...
@section('content')
<div class="max-w-3xl">
    <form action="{{ route('faqs.store') }}" method="POST" class="bg-white rounded-2xl-custom border border-off-white shadow-custom p-8">
        @csrf
        <div class="space-y-6">
            <div>
                <label class="block text-xs font-black uppercase tracking-widest text-navy mb-2">Pertanyaan</label>
                <input type="text" name="question" value="{{ old('question') }}" class="w-full px-4 py-3 rounded-xl border border-off-white bg-off-white/30 focus:outline-none focus:ring-2 focus:ring-gold/20 focus:border-gold transition-all" placeholder="Apa yang ingin ditanyakan?">
            ...
            <div>
                ...
                <textarea name="answer" rows="5" class="w-full px-4 py-3 rounded-xl border border-off-white bg-off-white/30 focus:outline-none focus:ring-2 focus:ring-gold/20 focus:border-gold transition-all" placeholder="Berikan jawaban yang jelas...">{{ old('answer') }}</textarea>
        ...
        <div class="mt-8 flex justify-end gap-3">
            ...
            <button type="submit" class="px-8 py-3 bg-gradient-to-r from-navy to-navy-light text-white font-bold text-xs rounded-xl hover:shadow-lg transition-all border border-gold/10 uppercase tracking-widest">Simpan FAQ</button>
        </div>
    </form>
</div>
@endsection
