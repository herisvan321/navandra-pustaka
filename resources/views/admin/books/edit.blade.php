...
<div class="content-header" style="margin-bottom: 25px;">
    <div style="display: flex; align-items: center; gap: 15px;">
        ...
        <h2 style="font-size: 1.5rem; font-weight: 700;">Edit Data Buku</h2>
    </div>
</div>
    ...
    <form action="{{ route('admin.books.update', $book->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="grid-3">
            <div class="form-group">
                ...
                <input type="text" name="title" value="{{ old('title', $book->title) }}" required placeholder="Judul buku..." class="@error('title') invalid @enderror">
                @error('title') <span class="error-text">{{ $message }}</span> @enderror
            </div>
            <div class="form-group">
                <label>Penulis</label>
                <input type="text" name="author" value="{{ old('author', $book->author) }}" required placeholder="Nama penulis..." class="@error('author') invalid @enderror">
                @error('author') <span class="error-text">{{ $message }}</span> @enderror
            </div>
            <div class="form-group">
                <label>ISBN</label>
                <input type="text" name="isbn" value="{{ old('isbn', $book->isbn) }}" placeholder="978-..." class="@error('isbn') invalid @enderror">
        ...
        <div class="grid-2" style="margin-top: 20px;">
            <div class="form-group">
                ...
                <input type="text" name="category" value="{{ old('category', $book->category) }}" placeholder="E.g. Fiksi, Sejarah..." class="@error('category') invalid @enderror">
                @error('category') <span class="error-text">{{ $message }}</span> @enderror
            </div>
            <div class="form-group">
                <label>Harga (Rp)</label>
                <input type="number" name="price" value="{{ old('price', $book->price) }}" placeholder="0" class="@error('price') invalid @enderror">
        ...
        <div class="form-group" style="margin-top: 20px;">
            ...
            <textarea name="description" rows="5" placeholder="Sinopsis singkat..." class="@error('description') invalid @enderror">{{ old('description', $book->description) }}</textarea>
        ...
        <div class="grid-2" style="margin-top: 20px;">
            <div class="form-group">
                ...
                <div class="image-preview-wrapper" onclick="document.getElementById('coverInput').click()" style="cursor: pointer;">
                    <div class="preview-placeholder" id="placeholder" style="{{ $book->cover_image ? 'display: none;' : '' }}">
                        <i class="fas fa-cloud-upload-alt"></i>
                        <span>Pilih Sampul Buku</span>
                    </div>
                    <img id="coverPreview" src="{{ $book->cover_image ? asset('storage/' . $book->cover_image) : '' }}" class="preview-img" style="{{ $book->cover_image ? '' : 'display: none;' }}">
                </div>
            ...
            <div class="form-group">
                ...
                <select name="status" class="@error('status') invalid @enderror">
                    ...
                    <option value="out_of_stock" {{ old('status', $book->status) == 'out_of_stock' ? 'selected' : '' }}>Habis</option>
                    <option value="pre_order" {{ old('status', $book->status) == 'pre_order' ? 'selected' : '' }}>Pre-Order</option>
                </select>
