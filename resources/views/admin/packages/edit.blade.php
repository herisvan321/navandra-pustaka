...
<div class="content-header" style="margin-bottom: 25px;">
    <div style="display: flex; align-items: center; gap: 15px;">
        ...
        <h2 style="font-size: 1.5rem; font-weight: 700;">Edit Paket</h2>
    </div>
</div>
    ...
    <form action="{{ route('admin.packages.update', $package->id) }}" method="POST">
        ...
        <div class="grid-2" style="grid-template-columns: 2fr 1fr; margin-top: 20px;">
            <div class="form-group">
                ...
                <input type="text" name="name" value="{{ old('name', $package->name) }}" required placeholder="E.g. Paket Hemat, Paket Pro..." class="@error('name') invalid @enderror">
                @error('name') <span class="error-text">{{ $message }}</span> @enderror
            </div>
            <div class="form-group">
                <label>Harga (Rp)</label>
                <input type="number" name="price" value="{{ old('price', $package->price) }}" required placeholder="0" class="@error('price') invalid @enderror">
        ...
        <div class="form-group" style="margin-top: 20px;">
            ...
            <textarea name="description" rows="3" placeholder="Penjelasan singkat paket..." class="@error('description') invalid @enderror">{{ old('description', $package->description) }}</textarea>
        ...
        <div class="form-group" style="margin-top: 25px; display: flex; align-items: center; gap: 10px;">
            <input type="checkbox" name="is_featured" id="isFeatured" {{ old('is_featured', $package->is_featured) ? 'checked' : '' }} style="width: auto;">
