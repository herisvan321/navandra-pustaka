...
<div class="content-header" style="margin-bottom: 25px;">
    <div style="display: flex; align-items: center; gap: 15px;">
        ...
        <h2 style="font-size: 1.5rem; font-weight: 700;">Edit Event</h2>
    </div>
</div>
...
<div class="card" style="max-width: 800px; margin: 0 auto;">
    <form action="{{ route('admin.events.update', $event->id) }}" method="POST">
        ...
        <div class="form-group">
            ...
            <input type="text" name="title" value="{{ old('title', $event->title) }}" required placeholder="E.g. Lomba Menulis Cerpen Nasional..." class="@error('title') invalid @enderror">
        ...
        <div class="grid-2" style="margin-top: 20px;">
            <div class="form-group">
                ...
                <select name="type" class="@error('type') invalid @enderror">
                    <option value="competition" {{ old('type', $event->type) == 'competition' ? 'selected' : '' }}>Kompetisi (Lomba)</option>
            ...
            <div class="form-group">
                ...
                <input type="date" name="deadline" value="{{ old('deadline', $event->deadline ? \Carbon\Carbon::parse($event->deadline)->format('Y-m-d') : '') }}" required class="@error('deadline') invalid @enderror">
        ...
        <div class="form-group" style="margin-top: 20px;">
            ...
            <textarea name="description" rows="5" placeholder="Tuliskan detail event, syarat, dan prasyarat..." class="@error('description') invalid @enderror">{{ old('description', $event->description) }}</textarea>
            @error('description') <span class="error-text">{{ $message }}</span> @enderror
        </div>

        <div class="form-group">
            <label>Link Pendaftaran / Informasi</label>
            <input type="url" name="link" value="{{ old('link', $event->link) }}" placeholder="https://..." class="@error('link') invalid @enderror">
        ...
        <div class="form-group" style="margin-top: 25px; display: flex; align-items: center; gap: 10px;">
            <input type="checkbox" name="is_active" id="isActive" {{ old('is_active', $event->is_active) ? 'checked' : '' }} style="width: auto;">
